...
export default class Forgotpwd extends Component {
   ...
   forgotPassword=()=>{
      if(this.state.email === ""){
         ...
         let details = {email:this.state.email}
         api.post(Url+"/api/users/forgotpwd", details)
         .then((response) => {
            NotificationManager.success('Please check your mail');
            this.setState({
               emailError:false
             })
         })
                        ...
                        <div className="session-body text-center">
                           ...
                           <Form>
                              <FormGroup className="has-wrapper">
                                 ...
                                 {this.state.emailError ? (<span className="error">{"Enter valid email"}</span>):null}
                              </FormGroup>
                              <FormGroup>
                                 <Button variant="contained" className="btn-info text-white btn-block btn-large w-100" onClick={this.forgotPassword}>word</Button>
                              </FormGroup>
